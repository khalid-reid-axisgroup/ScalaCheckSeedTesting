...
private[tools] object ArgsParser {
  ...
  private[scalatest] def checkArgsForValidity(args: Array[String]) = {
    ...
    while (it.hasNext) {
      ...
      if (s.startsWith("-r"))
        ...
        !s.startsWith("-D") && !s.startsWith("-g") && !s.startsWith("-o") && !s.startsWith("-e") && !s.startsWith("-P")
      ) {
        lb += s
      }
    }
    ...
    val (numThreads, enableSuiteSortingReporter) = threadOpt match {
      ...
      case Some(arg) =>
        ...
        if (numT > 0)
          ...
          throw new IllegalArgumentException(
            "-P with negative or zero thread number is invalid, please pass in a positive thread number instead."
          )
      ...
      /*else if (s.startsWith("-d")) {
        ...
        if (it.hasNext) {
          if (isParsableAsInt(it.head)) slowpoke += it.next()
            ...
            throw new IllegalArgumentException(
              "-W must be followed by a valid integer specifying the delay, but got: " + it.head
            )
        ...
        if (it.hasNext) {
          if (isParsableAsInt(it.head)) slowpoke += it.next()
            ...
            throw new IllegalArgumentException(
              "-W must be followed by two valid integers, the second specifying the period, but got: " + it.head
            )
        } else
          throw new IllegalArgumentException(
            "-W must be followed by two valid integers, the second specifying the period"
          )
        // SKIP-SCALATESTJS,NATIVE-END
        // SCALATESTJS,NATIVE-ONLY throw new IllegalArgumentException("Argument not supported by ScalaTest-js: " + s)
      } else {
    ...
    while (it.hasNext) {
      ...
      if (it.hasNext) {
        ...
        if (!suiteName.startsWith("-"))
          ...
          throw new IllegalArgumentException(
            "Expecting a Suite class name or package name to follow " + dashArg + ", but got: " + suiteName
          )
      } else
        throw new IllegalArgumentException(
          "Last element must be a Suite class name or package name, not a " + dashArg + "."
        )
    }
    lb.toList
  }

  /** Returns a possibly empty ConfigSet containing configuration objects specified in the passed reporterArg.
    * Configuration options are specified immediately following the reporter option, as in:
    *
    * -oFA
    *
    * If no configuration options are specified, this method returns an empty ConfigSet. This method never returns null.
    */
  def parseConfigSet(reporterArg: String): Set[ReporterConfigParam] = {
    ...
    while (it.hasNext)
      it.next match {
        case 'Y' =>
          throw new IllegalArgumentException(
            "Use of Y was deprecated in ScalaTest 1.0 and removed in 1.5. Please check the Scaladoc documentation of org.scalatest.Runner for information on valid Reporter config parameters."
          )
        case 'Z' =>
          throw new IllegalArgumentException(
            "Use of Z was deprecated in ScalaTest 1.0 and removed in 1.5. Please check the Scaladoc documentation of org.scalatest.Runner for information on valid Reporter config parameters."
          )
        ...
        case 'B' =>
          throw new IllegalArgumentException(
            "Use of B was deprecated in ScalaTest 1.0 and removed in 1.5. Please check the Scaladoc documentation of org.scalatest.Runner for information on valid Reporter config parameters."
          )
        // case 'S' => // Use for Short Stack Traces
        case 'A' =>
          throw new IllegalArgumentException(
            "Use of A was deprecated in ScalaTest 1.0 and removed in 1.5. Please check the Scaladoc documentation of org.scalatest.Runner for information on valid Reporter config parameters."
          )
  ...
  def parseReporterArgsIntoConfigurations(args: List[String]) = {
      ...
      args.find(arg => arg.startsWith("-g")) match {
        case Some(dashGString) =>
          ...
          if (configSet.contains(PresentShortStackTraces))
            throw new IllegalArgumentException(
              "Cannot specify an S (present short stack traces) configuration parameter for the graphic reporter (because it shows them anyway): " + dashGString
            )
          if (configSet.contains(PresentFullStackTraces))
            throw new IllegalArgumentException(
              "Cannot specify an F (present full stack traces) configuration parameter for the graphic reporter (because it shows them anyway): " + dashGString
            )
          if (configSet.contains(PresentWithoutColor))
            throw new IllegalArgumentException(
              "Cannot specify a W (present without color) configuration parameter for the graphic reporter: " + dashGString
            )
          if (configSet.contains(PresentAllDurations))
            throw new IllegalArgumentException(
              "Cannot specify a D (present all durations) configuration parameter for the graphic reporter (because it shows them all anyway): " + dashGString
            )
          if (configSet.contains(PresentUnformatted))
            throw new IllegalArgumentException(
              "Cannot specify a U (present unformatted) configuration parameter for the graphic reporter: " + dashGString
            )
          if (configSet.contains(PresentFilePathname))
            throw new IllegalArgumentException(
              "Cannot specify a V (present file pathname) configuration parameter for the graphic reporter: " + dashGString
            )
    ...
    def buildJunitXmlReporterConfigurationList(args: List[String]) = {
      ...
      while (it.hasNext) {
        val arg = it.next
        if (arg.startsWith("-u"))
          lb += new JunitXmlReporterConfiguration(Set[ReporterConfigParam](), it.next)
      }
      lb.toList
    }
    ...
    def buildHtmlReporterConfigurationList(args: List[String]): List[HtmlReporterConfiguration] =
      if (args.isEmpty)
        Nil
    ...
    //
    def parseHtmlArgs(args: List[String]): (Set[ReporterConfigParam], String, Option[URL], List[String]) = {
    ...
    def buildCustomReporterConfigurationList(args: List[String]) = {
      ...
      while (it.hasNext) {
        ...
        if (arg.startsWith("-C")) {
          ...
          if (configSet.contains(PresentShortStackTraces))
            throw new IllegalArgumentException(
              "Cannot specify an S (present short stack traces) configuration parameter for a custom reporter: " + dashCString + " " + customReporterClassName
            )
          if (configSet.contains(PresentFullStackTraces))
            throw new IllegalArgumentException(
              "Cannot specify an F (present full stack traces) configuration parameter for a custom reporter: " + dashCString + " " + customReporterClassName
            )
          if (configSet.contains(PresentWithoutColor))
            throw new IllegalArgumentException(
              "Cannot specify a W (without color) configuration parameter for a custom reporter: " + dashCString + " " + customReporterClassName
            )
          if (configSet.contains(PresentAllDurations))
            throw new IllegalArgumentException(
              "Cannot specify a D (present all durations) configuration parameter for a custom reporter: " + dashCString + " " + customReporterClassName
            )
          if (configSet.contains(PresentUnformatted))
            throw new IllegalArgumentException(
              "Cannot specify a U (present unformatted) configuration parameter for a custom reporter: " + dashCString + " " + customReporterClassName
            )
          if (configSet.contains(PresentFilePathname))
            throw new IllegalArgumentException(
              "Cannot specify a V (present file pathname) configuration parameter for a custom reporter: " + dashCString + " " + customReporterClassName
            )
  ...
  private[scalatest] def parseSuiteArgs(args: List[String]): (List[SuiteParam], List[TestSpec]) = {
    ...
    while (it.hasNext) {
      ...
      if (!it.hasNext)
        throw new IllegalArgumentException("argument " + dashArg + " must be followed by a name")

      val argVal = it.next

      if (argVal.startsWith("-"))
        throw new IllegalArgumentException("Expecting a name to follow " + dashArg + ", but got [" + argVal + "]")
          ...
          val nestedSuites =
            if (it.hasNext && it.head == "-i") {
              ...
              while (it.hasNext && it.head == "-i") {
                ...
                nestedLb += new NestedSuiteParam(
                  suiteId,
                  suiteIdTestNamesBuffer.toArray,
                  suiteIdWildcardTestNamesBuffer.toArray
                )
              }
              nestedLb.toArray
            } else
  ...
  def parseAgainArgs(args: List[String]): List[String] = {
    ...
    while (it.hasNext) {
      ...
      if (!it.hasNext)
        throw new IllegalArgumentException("-A argument must be followed by a file name")
    ...
    if (args.length == 0) {
      ...
      def parsePair(dashArg: String, compoundArg: String) = {
        ...
        if (compoundArg.trim.isEmpty)
          throw new IllegalArgumentException(
            "The argument string must actually include some non-whitespace characters."
          )
      ...
      if (it.hasNext) {
        ...
        try
          ...
          case e: NumberFormatException =>
            throw new IllegalArgumentException(
              dashArg + " must be followed by a number, but '" + spanString + "' is not a number."
            )
        }
      } else
